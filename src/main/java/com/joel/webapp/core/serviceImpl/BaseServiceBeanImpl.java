...
public abstract class BaseServiceBeanImpl<T, PK extends Serializable> implements BaseServiceBean<T, PK> {
    ...
    @Override
    public synchronized boolean deleteOne(PK id) {
        this.getDao().deleteOne(id);
        return true;
    }

    @Override
    public synchronized boolean deleteOne(T t) {
    ...
    public void delete(T entity){
        try {
            ...
            Logger.getLogger(BaseServiceBeanImpl.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
